...
class Engine {
  ...
  render(ctx) {
    return this.engines[ctx.locale].render({
      documentFilePath: path.join(this.options.browser, name, this.options.index),
      url: ctx.url,
  ...
  renderFile(filename, locals, options = {}) {
    ...
    return this.engines[locals.ctx.locale].render(options);
  }
